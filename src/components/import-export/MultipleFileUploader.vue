<template>
    <div class="uploadBox">
        ...
        <form role="form" enctype="multipart/form-data" @submit.prevent="onSubmit">
            ...
            <div>
                <v-btn color="warning" type="submit" :disabled="itemsAdded < minItems || itemsAdded > maxItems">
                    {{ uploadButtonMessage }}
                </v-btn>
                <v-btn color="error" @click="removeItems" :disabled="itemsAdded < minItems">
                  {{ cancelButtonMessage }}
                </v-btn>
            </div>
        </form>
...
export default {
    ...
    methods: {
        ...
        onChange(e) {
            ...
            this.formData.append('project', files[0]);
        },
        removeItems() {
            this.items = '';
            this.itemsAdded = '';
